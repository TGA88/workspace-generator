...
cp -r $GENERATOR_DIR/script-generator/template/project/base-types/* $WORKSPACE_DIR/workspaces/$SYSTEM_DIR/libs/$PROJECT_NAME
...
if [[ "$OSTYPE" == "darwin"* ]]; then
    ...
    cut -d: -f1 | 
    xargs sed -i '' -e "s/base-types/$PROJECT_NAME/g" -e "s/feedos-example-system/$WORKSPACE_DIR/g"
...
echo "Replaced 'base-types' with '$PROJECT_NAME' in all files under $WORKSPACE_DIR/workspaces/$SYSTEM_DIR/libs/$PROJECT_NAME"
