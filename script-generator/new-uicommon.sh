...
cp -r $GENERATOR_DIR/script-generator/template/project/ui-common/* $WORKSPACE_DIR/workspaces/$SYSTEM_DIR/libs/$PROJECT_NAME/
...
if [[ "$OSTYPE" == "darwin"* ]]; then
    ...
    cut -d: -f1 | 
    xargs sed -i '' -e "s/ui-exm/$PROJECT_NAME/g" -e "s/feedos-example-system/$WORKSPACE_DIR/g"
    # xargs sed -i '' "s/@ui-exm/@$PROJECT_NAME/g"
...
echo "Replaced 'ui-exm' with '$PROJECT_NAME/' in all files under $WORKSPACE_DIR/workspaces/$SYSTEM_DIR/libs/$PROJECT_NAME/"
