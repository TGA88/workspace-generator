...
#    echo "Error: PROJECT_NAME is not set"
    PROJECT_NAME='common-functions'
    echo "PROJECT_NAME: $PROJECT_NAME"
else
    PROJECT_NAME=${PROJECT_NAME}-functions
fi
...
cp -r $GENERATOR_DIR/script-generator/template/project/functions/* $WORKSPACE_DIR/workspaces/$SYSTEM_DIR/libs/$PROJECT_NAME
...
if [[ "$OSTYPE" == "darwin"* ]]; then
    ...
    cut -d: -f1 | 
    xargs sed -i '' -e "s/common-functions/$PROJECT_NAME/g" -e "s/feedos-example-system/$WORKSPACE_DIR/g"
...
echo "Replaced 'common-functions' with '$PROJECT_NAME' in all files under $WORKSPACE_DIR/workspaces/$SYSTEM_DIR/libs/$PROJECT_NAME"
