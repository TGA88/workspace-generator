...
export const processGet__Domain__InRepoHandler: UnifiedHandlerFn = async (context) => {
  ...
  const repo = getRegistryItem<Repository>(
    context,
    __DOMAINUP___API_CONTEXT_KEY.REPO_GET___DOMAINUP__,
  ) as Repository;

  const result = await repo.get__Domain__(context, { id: inputRequest.id });
  if (result.isFailure) {
    logger.error('get-__DOMAIN__ fail', new Error(String(result.errorValue())));
...
// get-__DOMAIN__ ไม่มี pure logic แยก -> ไม่มี business.logic; setupProcess อยู่กับ steps
export function setupProcess(): { preHandlers: UnifiedPreHandlerFn[]; handler: UnifiedHandlerFn } {
  return {
    ...
    handler: processGet__Domain__InRepoHandler,
  };
}
