...
} from '@__WS__/__API__-core/__domain__-api/command/__verb__-__domain__';
...
import { validate__Verb____Domain__Input } from './business.logic';
...
export const processCheckRequiredFieldPreHandler: UnifiedHandlerFn = async (context) => {
  ...
  const validationResult = validate__Verb____Domain__Input(inputRequest as __Verb____Domain__Input);
  if (!validationResult.isValid) {
    logger.error('validate-__verb__-__domain__-input fail', new Error(validationResult.message));
...
export const process__Verb____Domain__Handler: UnifiedHandlerFn = async (context) => {
  ...
  const result = await repo.__verb____Domain__(context, inputRequest as __Verb____Domain__Input);
  if (result.isFailure) {
