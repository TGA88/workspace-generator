    ...
    fastify.addHook('preHandler', async (req, reply) => {
        try {
          ...
          prismaClient.aCCOUNT_MAS_COUNTRY_FARM
            ...
            const mapCVCode = accountMasCountryFarm.map((data ) => data.MAS_COUNTRY_FARM.CV_CODE as string)
            if (mapCVCode.length > 0) {
                ...
                countryCode = groupCountryCode.map((data) => data.COUNTRY_CODE as string)
                ...
                speciesCode = groupSpeciesCode.map((data) => data.SPECIES_CODE as string)
            }
            else {
                ...
                countryCode = accountMasCountry.map((data) => data.MAS_COUNTRY.COUNTRY_CODE)
                ...
                speciesCode = accountMasSpecies.map((data) => data.SPECIES_CODE)
            }
            ...
            req.userPermission.orgCode = accountOrg.map((data) => data.ORG_CODE)
            req.userPermission.buCode = accountBu.map((data) => data.BU_CODE)
            req.userRole = accountRole.map((data) => data.ROLE_CODE)
        } catch (error) {
